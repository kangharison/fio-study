/*
 * [한국어 설명] axmap(계층적 비트맵) 테스트 프로그램 (axmap.c)
 *
 * === 파일의 역할 ===
 * lib/axmap.c에 구현된 계층적 비트맵(axmap) 자료구조의 단위 테스트 프로그램이다.
 * LFSR로 생성한 랜덤 값을 사용하여 비트 설정, 중복 감지, 다음 빈 비트 탐색 등
 * axmap의 핵심 기능이 정상적으로 동작하는지 검증한다.
 *
 * === 사용법 ===
 * 컴파일 후 직접 실행하면 다양한 크기의 비트맵에 대해
 * 자동화된 테스트를 수행하고 결과를 출력한다.
 */
#include <stdio.h>
#include <stdlib.h>
#include <inttypes.h>
