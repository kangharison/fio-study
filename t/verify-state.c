/*
 * [한국어 설명] 검증 상태 파일 리더 (verify-state.c)
 *
 * === 파일의 역할 ===
 * fio의 검증(verify) 상태 파일을 읽어 사람이 읽을 수 있는 텍스트 형식으로
 * 덤프하는 디버깅 도구이다. 스레드별 I/O 깊이, 인덱스, 진행 중인 쓰기 정보
 * 등을 출력하여 검증 상태를 확인할 수 있다.
 *
 * === 사용법 ===
 * fio가 생성한 검증 상태 파일 경로를 인자로 지정하여 실행하면
 * 해당 파일의 내용을 텍스트로 출력한다.
 */
/*
