/*
 * [한국어 설명] LFSR 테스트 프로그램 (lfsr-test.c)
 *
 * === 파일의 역할 ===
 * 선형 피드백 시프트 레지스터(LFSR) 구현을 테스트하는 프로그램이다.
 * 지정된 수의 난수를 생성하고, 선택적으로 LFSR이 모든 값을 정확히 한 번씩
 * 순회하는지 검증하며, 생성 속도(경과/평균 시간)를 측정한다.
 *
 * === 사용법 ===
 * 생성할 난수 개수를 16진수로 지정하여 실행한다.
 * 예: ./lfsr-test 0x10000 [seed] [spin] [verify]
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
