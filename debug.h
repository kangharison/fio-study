/*
 * [한국어] debug.h - 디버그 출력 시스템 헤더
 *
 * fio의 디버그 로깅 인프라를 정의한다.
 * 디버그 카테고리별 비트마스크를 사용하여 선택적으로 디버그 출력을 활성화할 수 있다.
 * 런타임에 --debug=io,verify 등으로 특정 카테고리만 활성화 가능.
 *
 * 주요 구성:
 *   - 디버그 카테고리 열거형 (FD_PROCESS, FD_IO 등)
 *   - 경고 중복 방지 비트마스크 (fio_did_warn)
 *   - dprint() 매크로 (비트마스크 기반 조건부 출력)
 */
#ifndef FIO_DEBUG_H
#define FIO_DEBUG_H
