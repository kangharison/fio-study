/*
 * [한국어 설명] 생산자-소비자 링 버퍼 단위 테스트 (pcbuf.c)
 *
 * === 파일의 역할 ===
 * pcbuf(producer-consumer ring buffer) 모듈의 링 버퍼 기능을 검증하는 단위 테스트이다.
 * 생산자가 데이터를 삽입하고 소비자가 꺼내는 과정에서 데이터 무결성과 순서가 보장되는지 테스트한다.
 * 버퍼 가득 참, 비어 있음 등의 경계 조건에서 올바르게 동작하는지 확인한다.
 */
/**
