/*
 * [한국어 설명] fio 서버/클라이언트 네트워크 프로토콜 정의 (server.h)
 *
 * === 파일의 역할 ===
 * 이 헤더는 fio의 서버 모드에서 사용되는 프로토콜 구조체, 명령 코드(opcode),
 * PDU 구조체, sk_out(소켓 출력 관리), 서버 API 함수 선언을 포함한다.
 * 와이어 인코딩은 리틀 엔디안이며, CRC16 체크섬과 프래그먼트 분할을 지원한다.
 *
 * === 전체 아키텍처에서의 위치 ===
 * server.c와 client.c 양쪽에서 이 헤더를 포함하여 프로토콜을 공유한다.
 * server.h → server.c(서버 구현) / client.c(클라이언트 구현)
 *
 * === 타 모듈과의 연결 ===
 * - server.c: 서버 측 프로토콜 구현 (수신, 디스패치, 전송)
 * - client.c: 클라이언트 측 프로토콜 구현 (연결, 전송, 수신)
 * - stat.h: thread_stat/group_run_stats를 PDU로 전송
 * - iolog.c: I/O 로그를 압축하여 전송
 *
 * === 주요 함수/구조체 요약 ===
 * - struct fio_net_cmd: 와이어 프로토콜 기본 단위 (헤더 + 페이로드)
 * - FIO_NET_CMD_*: 서버-클라이언트 통신 명령 코드 열거
 * - struct sk_out: 소켓 출력 관리 (참조 카운팅, 전송 큐)
 * - cmd_*_pdu: 각 명령의 페이로드 데이터 형식
 */
#ifndef FIO_SERVER_H
#define FIO_SERVER_H
