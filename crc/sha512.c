...
/*
 * [한국어 설명] SHA-512 해시 알고리즘 구현 (sha512.c)
 *
 * === 파일의 역할 ===
 * SHA-512(512비트 다이제스트)를 구현한다. SHA-2 계열 해시 함수로,
 * 1024비트(128바이트) 입력 블록을 80스텝으로 처리하여 512비트 해시를 생성한다.
 * 64비트 워드 연산을 사용하므로 64비트 CPU에서 SHA-256보다 빠를 수 있다.
 *
 * === 전체 아키텍처에서의 위치 ===
 * 호출 체인: verify.c → fio_sha512_init/update/final → sha512_transform
 *
 * === 주요 함수 요약 ===
 * - sha512_transform(): 1024비트 블록을 80스텝으로 처리하는 핵심 변환
 * - fio_sha512_init(): 8개 해시 워드를 SHA-512 초기값으로 설정
 * - fio_sha512_update(): 데이터를 128바이트 블록 단위로 변환 적용
 * - fio_sha512_final(): 패딩 추가 후 최종 해시 확정 (빅엔디안 출력)
 */
#include <string.h>
