 ...
 *
 * === 파일의 역할 ===
 * 바이트 수나 비트 수 같은 큰 숫자를 "1.5GiB" 또는 "100MB/s"와 같은 사람이 읽기 쉬운
 * 문자열로 변환한다. SI 접두사(k, M, G)와 IEC 접두사(Ki, Mi, Gi) 모두 지원하며,
 * 소수점 이하 반올림 처리도 포함한다.
 *
 * === fio에서의 사용 ===
 * fio의 통계 출력 포맷팅에 사용된다. 벤치마크 결과의 대역폭, IOPS, 지연 시간 등을
 * 사용자에게 보기 좋은 형태로 표시할 때 이 변환 함수들을 호출한다.
 */
#include <assert.h>
#include <stdlib.h>
#include <stdio.h>
