/*
 * [한국어 설명] memcpy 성능 테스트 (memcpy.c)
 *
 * === 파일의 역할 ===
 * 다양한 블록 크기(8바이트~512KB)에 대해 memcpy, memmove, 단순 복사, 하이브리드
 * 복사의 메모리 대역폭을 측정하는 벤치마크를 제공한다. 32MB 버퍼를 64회 반복
 * 복사하여 MiB/sec 단위로 결과를 출력한다.
 *
 * === fio에서의 사용 ===
 * fio 시작 시 --memcpy-test 옵션으로 시스템의 메모리 복사 성능을 측정할 수 있다.
 * 이를 통해 메모리 대역폭이 I/O 성능에 미치는 영향을 파악할 수 있다.
 */
#include <inttypes.h>
#include <stdio.h>
#include <stdlib.h>
