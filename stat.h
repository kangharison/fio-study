/*
 * [한국어 설명] fio 통계 수집/출력 시스템 헤더 (stat.h)
 *
 * === 파일의 역할 ===
 * 이 파일은 fio의 성능 측정 및 통계 보고 시스템의 구조체와 API를 정의한다.
 * group_run_stats(그룹 통계), thread_stat(스레드 통계), jobs_eta(진행률),
 * 퍼센타일 계산용 로그 스케일 히스토그램 상수, 통계 API 함수 선언을 포함한다.
 *
 * === 전체 아키텍처에서의 위치 ===
 * stat.c와 함께 통계 서브시스템을 구성한다. thread_data.ts 필드로 스레드별 통계가
 * 누적되며, fio.h에서 이 헤더를 포함하여 거의 모든 모듈에서 접근 가능하다.
 * stat.h → stat.c(구현) / server.c(네트워크 전송) / client.c(수신 및 출력)
 *
 * === 타 모듈과의 연결 ===
 * - stat.c: 이 헤더에 선언된 구조체와 함수의 구현
 * - fio.h: thread_data에 thread_stat ts 필드로 포함
 * - server.c/client.c: thread_stat/group_run_stats를 네트워크로 전송/수신
 * - io_u.c: add_*_sample()로 샘플 추가 시 thread_stat 갱신
 *
 * === 주요 함수/구조체 요약 ===
 * - struct group_run_stats: 잡 그룹 단위 실행 통계 (대역폭, 실행 시간 등)
 * - struct thread_stat: 스레드별 상세 통계 (레이턴시, IOPS, 대역폭, 퍼센타일 등)
 * - struct jobs_eta: 실행 중 ETA(진행률) 표시 정보
 * - FIO_IO_U_PLAT_*: 로그 스케일 퍼센타일 히스토그램 상수 (오차 < 0.78%)
 */
#ifndef FIO_STAT_H
#define FIO_STAT_H
